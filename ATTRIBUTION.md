...
translated to English, and the changes below were made. Each file carries an "Adapted from" header
naming its origin.

| File | Upstream path | What changed |
|---|---|---|
| `warehouse/macros/cross_db.sql` | same | Comments translated. Dispatch namespace changed from `quantai_warehouse` to `lp_lens_warehouse`. Three of the five macros dropped — `iso_day_of_week`, `year_month` and `local_date_from_utc` — because private markets data has no trading calendar and no intraday UTC timestamps. `day_spine` and `asof_left_join` kept, both unchanged in logic; the dispatch structure is kept as-is. Phase 2 added three original macros (`quarter_end_date`, `month_end_date`, `days_between`) in the same dispatch style; they are not from upstream. |
| `warehouse/macros/generate_schema_name.sql` | same | Comment translated and extended to name the `intermediate` layer, which upstream did not have. Phase 3 prefixes custom schemas when `target.name == 'snowflake_ci'` so a PR write cannot overwrite shared `STAGING`/`MARTS`. Other targets still return the custom schema unchanged. |
...
|---|---|---|
| **Layered dbt project.** `raw` → `staging` → `marts`, staging as views and marts as tables, materialization set once at directory level in `dbt_project.yml` rather than per model | `warehouse/dbt_project.yml` | Same layering and the same directory-level materialization, with an **`intermediate/` layer added** that upstream had none of. Upstream's `dbt_project.yml` and `profiles.yml` were deleted rather than edited, because both are named and pathed for the equities project. The models under `warehouse/models/{staging,intermediate,marts}/` are original. |
...
| **Integration test that seeds a warehouse, invokes `dbtRunner` programmatically, then reconciles SQL output against an independent Python implementation** | `tests/warehouse/test_dbt_build.py` | Same shape, implemented as `tests/test_marts_reconcile.py`: builds the DuckDB warehouse, recomputes DPI, RVPI, TVPI, net IRR and KS-PME from `data/raw/` in pandas, and includes a negative control that perturbs one cash flow. None of the upstream expected values or models apply. |
| **Snowflake raw load as a single transaction with rollback**, refusing to load an incomplete snapshot | `infra/snowflake/load_raw.py` | Same guarantee, rewritten for `PUT` to an internal stage plus `COPY INTO` inside `BEGIN`/`COMMIT`. Snapshot checks run on local Parquet (complete table set, 10-minute write window, referential integrity) before any Snowflake write. Upstream loaded from S3; this project has no data lake. |
| **Cost control**: XSMALL warehouse, `AUTO_SUSPEND = 60`, a resource monitor with a small monthly quota and a suspend trigger | `infra/snowflake/admin_setup.sql` | Same numbers (X-Small, 60s, 20 credits/month, suspend at 100%), applied in `infra/snowflake/setup.sql`, plus a LOADER / TRANSFORMER / REPORTER role split where upstream had a single `QUANTAI_DBT` role. One warehouse, not one per workload: the playbook asked for a single X-Small. |
...
alongside the notice for work added here. The four adapted files above remain subject to the
original notice.
